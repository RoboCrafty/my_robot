...
# Phone body axes (at Move-press, held flat/screen-up): +X right, +Y forward
# (top of phone), +Z up (out of the screen). Robot base frame: +X forward,
# +Y left, +Z up. This remap is the calibration between the two.
def _phone_to_robot(v: np.ndarray) -> np.ndarray:
    return np.array([v[1], -v[0], v[2]])


# --- Teleop main loop ----------------------------------------------------------
POSITION_GAIN = 1.5   # m of robot travel per m of phone travel
...
ANG_VEL_CAP = 2.0     # rad/s python-side clip (firmware's own cartrotvel caps further)
...
MOTION_DEADBAND = 0.005  # m/s or rad/s; below this, don't bother sending cartjogvel
                         # at all -- avoids spamming the firmware's UDP telemetry
                         # clients while idle, and drops position/rotation jitter
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if phone.is_connected:
            # Sending this every tick (even while idle) flooded the firmware's single
            # UDP telemetry-client slot, starving the browser web UI -- only resend
            # on a real change.
        ...
        if enabled:
            if not was_enabled:
                # Move just pressed: re-home axes to the phone's current orientation
                # and start tracking from here (first frame, nothing to differentiate).
                ...
                if dt >= 0.005:
                    ...
                    # World-frame delta rotation (current * last^-1); see teleop.py's
                    # earlier fix for why the reverse order is wrong.
                    delta_rotation = rot * last_rot.inv()
                    raw_ang_vel = delta_rotation.as_rotvec() / dt

                    # Rotate into the phone's frame at Move-press, then remap phone
                    # axes onto robot axes (see _phone_to_robot).
                    aligned_vel = home_rotation.apply(raw_vel)
                    aligned_ang_vel = home_rotation.apply(raw_ang_vel)
                    robot_vel = _phone_to_robot(aligned_vel) * POSITION_GAIN
                    robot_ang_vel = _phone_to_robot(aligned_ang_vel) * ROTATION_GAIN

                    # Low-pass filter the noisy per-frame derivative so small/slow
                    # in-place rotations aren't lost in jitter and motion is less jerky.
                    alpha = 1.0 - SMOOTHING
            ...
            # Move just released -- stop immediately.
